...
5. **Generate task files** in `queue/pending/`:
   ...
   - For the `branch:` field: if `BRANCH_PREFIX` is set in `craft.conf`, prepend it (e.g. `{{BRANCH_PREFIX}}feat/add-widget`). If not set, use a conventional branch name directly (e.g. `feat/add-widget`).
